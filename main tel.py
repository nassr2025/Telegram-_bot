# -*- coding: utf-8 -*-
"""
telegram_watcher.py  (main tel.py)

مراقب رسائل تيليجرام متعدد الجلسات باستخدام Telethon.
- يدعم تشغيل حتى 10 جلسات (أو أكثر) بالتوازي.
- قاعدة بيانات SQLite واحدة مشتركة (اتصال مستقل لكل خيط لتفادي "Recursive use of cursors not allowed").
- يحذف قاعدة البيانات تلقائيًا إذا تجاوز عمرها 24 ساعة (يُعاد إنشاؤها آليًا).
- فلترة ذكية: طول الرسالة، عدد الأسطر، تجاهل المشرفين، كلمات مفتاحية،
  تكرار عالمي/لكل مستخدم مع استثناء الرسائل الأكاديمية المهمة.
- تنسيق الإرسال بـ HTML مع روابط قابلة للنقر للمستخدم والرسالة.
- سجلات (Logs) واضحة لتتبع التشغيل على Render أو أي خادم.

قبل التشغيل محليًا:
    pip install telethon requests

على Render (خطة مجانية):
    - أرفق sessions.json (كـ Secret File أو داخل المستودع الخاص).
    - زد ملف app.py و render.yaml كما أرسلته لك.
"""
...
ALLOWED_IDS_FILE = "allowed_ids.json"  # اختياري: قائمة IDs للمُستلمين المسموح لهم (يتم دمجها مع الثابت أدناه)
DB_PATH = "seen.db"

# إرسال التنبيهات فقط لهؤلاء المستخدمين (ثابت داخل الكود — سيتم دمجه مع allowed_ids.json إن وُجد):
ALLOWED_RECIPIENTS_STATIC = [698815776, 7052552394]
...
def load_allowed_ids_file(path=ALLOWED_IDS_FILE):
    ...
    try:
        ...
        if isinstance(data, list):
            # نحاول تحويلها إلى أرقام صحيحة، ونستبعد غير الصالح
            ids = []
            for v in data:
                try:
                    ids.append(int(v))
        ...
        print(f"⚠️ [ALLOWED_IDS] خطأ قراءة {path}: {e}")
    return []

def effective_allowed_recipients():
    merged = list(ALLOWED_RECIPIENTS_STATIC) + load_allowed_ids_file()
    merged = _dedup(merged)
    if not merged:
        print("⚠️ [Recipients] لا يوجد أي مستلم مسموح! لن يتم إرسال تنبيهات.")
    else:
        print(f"📤 [Recipients] IDs المسموح لها: {merged}")
    return merged

# ===== تحميل / حفظ الجلسات =====
def load_sessions() -> List[str]:
    if not os.path.exists(SESS_FILE):
        print(f"❌ [Sessions] الملف {SESS_FILE} غير موجود.")
        return []
    try:
        with open(SESS_FILE, "r", encoding="utf-8") as f:
            ...
            if isinstance(data, list):
                ...
                print(f"📦 [Sessions] تم تحميل {len(uniq)} جلسة.")
                return uniq
    except Exception as e:
...
def save_sessions_list(sessions):
    try:
        ...
        print(f"💾 [Sessions] تم الحفظ — {len(sessions)} جلسة.")
    except Exception as e:
        print(f"⚠️ [Sessions] فشل الحفظ: {e}")
...
def validate_sessions(api_id, api_hash, sessions):
    ...
    for i, s in enumerate(sessions, 1):
        try:
            ...
            c.disconnect()
            valid.append(s)
            print(f"  ✅ جلسة #{i} صالحة")
        except AuthKeyUnregisteredError:
            invalid.append(s); print(f"  ⛔ جلسة #{i} غير صالحة (AuthKeyUnregistered)")
        except Exception as e:
            invalid.append(s); print(f"  ⛔ جلسة #{i} خطأ: {e}")
    if invalid:
    ...
    print(f"✅ [Sessions] عدد الجلسات الصالحة: {len(valid)}")
    return valid
...
def send_alert_http(text: str):
    recipients = effective_allowed_recipients()
    if not recipients:
        return
    try:
        ...
        for rid in recipients:
            for p in parts:
                try:
                    ...
                    print(f"⚠️ [Alerts] فشل إرسال التنبيه لـ {rid}: {e}")
                time.sleep(0.3)
    except Exception as e:
        print(f"⚠️ [Alerts] فشل عام: {e}")
...
def get_message_handler(db_conn):
    ...
    async def handle_message(event):
        try:
            ...
            print(f"🚨 [Alert] تم إرسال تنبيه — SenderID:{sender_id} Chat:{event.chat_id} Msg:{msg_id}")

        except Exception as e:
            print(f"⚠️ [Handler] خطأ: {e}")
...
def client_runner(session_str: str, idx: int):
    ...
    while True:
        try:
            ...
            print(f"✅ [{name}] متصل — يبدأ الاستماع للرسائل")
            send_alert_http(f"<b>✅ [{name}] متصل — بدأ الاستماع للرسائل.</b>")
            client.run_until_disconnected()
            print(f"ℹ️ [{name}] انتهى الاتصال — إعادة محاولة خلال {backoff}s")
...
def main():
    ...
    print(f"⚙️ الإعدادات: MAX_AD_LENGTH={MAX_AD_LENGTH}, MAX_LINES={MAX_LINES}, DUP_WINDOW={DUP_WINDOW_SECONDS}s")
    if ALLOWED_CHAT_USERNAMES:
        print(f"🔒 مراقبة محدودة على: {ALLOWED_CHAT_USERNAMES}")
    else:
        print("🌐 مراقبة كل المجموعات/القنوات المشترك بها الحسابات.")
    ...
    print(f"✅ تم تشغيل {len(threads)} جلسة. البوت الآن في وضع الاستماع.")
    try:
        while True:
            time.sleep(3600)
    except KeyboardInterrupt:
        print("👋 تم الإيقاف اليدوي.")
